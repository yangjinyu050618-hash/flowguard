...
### 1.2 Adapter Layer
FlowGuard provides non-intrusive async adapters for popular LLM client libraries (\openai\, \nthropic\, \google-genai\).
...
4. **Demanding Zero-Leak Async Cancellation**: When web requests (e.g. FastAPI / Starlette) are aborted by clients, in-flight rate limiter slots, bulkhead queues, and circuit breaker half-open probes must be immediately released without resource leaks.
...
2. **Synchronous-Only Codebases**: FlowGuard is built ground-up on Python \syncio\. Synchronous codebases should use thread-based tools like \	enacity\ or \pybreaker\.
...
Developers often attempt to assemble resilience by gluing together independent libraries (e.g., \	enacity\ for retries, \iolimiter\ for rate limiting, and ad-hoc \	ry/except\ for fallback). The table below compares the engineering characteristics:
...
| **Cancellation Safety** | Cancelling a task waiting on \	enacity.retry\ or custom locks can orphan queue slots or leave rate limiter tokens permanently drained. | **Zero-Leak Cancellation**: \syncio.CancelledError\ immediately unwinds token bucket waiters, bulkhead active/queue counters, and half-open probe slots. |
...
| **Fallback Context Decoupling** | Fallback functions often suffer from \*args, **kwargs\ keyword collisions (e.g. caller parameter named \xc\ colliding with exception payload). | **Typed \FallbackContext\**: Immutable context wrapping \(exception, args, kwargs, pipeline_name)\ with \MappingProxyType\, completely immune to keyword collisions. |
| **Candidate Execution Safety** | Using \	ry...except TypeError:\ to adapt dynamic fallback candidate signatures risks catching internal business bugs and executing candidate twice. | **Static Signature Inspection**: Reflects signatures outside execution blocks; sync and async candidate exceptions execute strictly once. |
...
| **Dependency & Maintenance** | 4–6 separate external libraries with differing async semantics, maintenance cycles, and potential version conflicts. | **Zero External Dependencies**: Pure standard-library async Python (\syncio\, \dataclasses\, \	yping\, \	ime\), zero bloat. |
